...
local function Create(table, spawn)
	local objects = {}
    local POIOffsets = {}
	POIOffsets.exit = json.decode(table.JSON['exit'])
	if type(table.JSON.apartmentAdditions)  == "table" then
		POIOffsets.clothes = json.decode(table.JSON.apartmentAdditions['clothes'])
		POIOffsets.stash = json.decode(table.JSON.apartmentAdditions['stash'])
		POIOffsets.logout = json.decode(table.JSON.apartmentAdditions['logout'])
	end
    DoScreenFadeOut(500)
    while not IsScreenFadedOut() do
        Wait(10)
    end
	RequestModel(table.model)
	while not HasModelLoaded(table.model) do
	    Wait(3)
	end
...
for export, data in Config do
	exports(export, function(spawn)
		Create(data, spawn)
	end)
end
